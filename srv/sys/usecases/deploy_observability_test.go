...
func TestDeployObservability_ExecutePersistentWithVolume(t *testing.T) {
	mockSSH := mocks.NewMockSSHExecutor()
	uc := NewDeployObservabilityUseCase(mockSSH)

	customPath := "/mnt/ext_storage/observability"
	err := uc.ExecutePersistentWithVolume(true, "single-node", "securepass123", customPath)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	allCmds := strings.Join(mockSSH.CommandsExecuted, " ||| ")
	if !strings.Contains(allCmds, customPath) {
		t.Errorf("expected custom volume path '%s' in executed commands, got: %s", customPath, allCmds)
	}
}
